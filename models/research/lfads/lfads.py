...
#                             in_mat_cxf.shape[1]))
#       22.12.25. Jungwoo Found Input matrix has proper size in h5 file. But causing an error.
#
            print("Alignment matrix must have dimensions %d x %d (data_dim x factors_dim)" \
              ",but currently has %d x %d. So changing it to proper size." \
               % (data_dim, factors_dim, in_mat_cxf.shape[0], in_mat_cxf.shape[1]))
            in_mat_cxf = in_mat_cxf.transpose()
