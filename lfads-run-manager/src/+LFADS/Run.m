classdef Run < handle & matlab.mixin.CustomDisplay
    ...
    methods
        ...
        function out = generateCountsForDataset(r, dataset, mode, varargin) %#ok<STOUT,INUSD>
            ...
            %   will be exported for LFADS, or 'alignment' indeicating that this
    ...
    methods
        ...
        function seq = convertDatasetToSequenceStruct(r, dataset, mode, varargin)
            ...
            binWidthMs = 1;% timeVecMs(2) - timeVecMs(1);
    ...
    methods
        ...
        function out = loadInputInfo(r, varargin)
            ...
            % fnames = cellfun(@(x) fullfile(r.pathCommonData, x), r.lfadsInputInfoFileNames(datasetIdx), 'UniformOutput', false);
            if ismac
                fnames = strrep(fnames, '/home/jungwoo', '/Users/jungwookim');
            end
...
%                 r.assertParamsOkayForSequenceData(seqData);
        ...
        function [pms, valid] = loadPosteriorMeans(r, varargin)
            ...
            if ismac 
                rlfadsout = strrep(r.pathLFADSOutput, '/home/jungwoo/', '/Users/jungwookim/');              %%%% 
            else
                rlfadsout = r.pathLFADSOutput;
            end
            
            for iiDS = 1:numel(datasetIdx)
                prog.update(iiDS);
                if ~exist(fullfile(rlfadsout, trainList{iiDS}), 'file')
                    oldFile = strrep(trainList{iiDS}, 'posterior_sample_and_average', 'posterior_sample');
                    if exist(fullfile(rlfadsout, oldFile), 'file')
                        trainList{iiDS} = oldFile;
                    else
                        warning('LFADS Posterior Mean train file not found for dataset %d: %s', ...
                            iiDS, fullfile(rlfadsout, trainList{iiDS}));
                        pms(iiDS) = pmConstructorFn(); %#ok<AGROW>
                        continue;
                    end
                end
                if ~exist(fullfile(rlfadsout, validList{iiDS}), 'file')
                ...
                pms(iiDS) = pmConstructorFn(...
                    fullfile(rlfadsout, validList{iiDS}), ...
                    fullfile(rlfadsout, trainList{iiDS}), ...
